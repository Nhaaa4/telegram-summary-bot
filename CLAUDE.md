...
- `services/llm_client.py` — `SummaryClient.summarize()` dispatches synchronously (wrapped in `asyncio.to_thread`) to one of: gemini (with key rotation/fallback across `gemini_api_keys`), openai, openrouter, hashn0de, deepseek, anajak, ollama, huggingface — all except gemini go through the `openai` SDK client pointed at a different `base_url`. Adding a provider means adding a branch here, a matching branch in `graph/agent.py`'s `build_chat_model` (used by the mention/DM agent), plus a `Literal` entry in `configs/settings.py`.
